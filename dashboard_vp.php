<?php 
	...
	$query = mysqli_query($connection, "SELECT 
											(SELECT COUNT(id) FROM `request_form` WHERE `status` = 'FOR VP') AS `request`,
											(SELECT COUNT(id) FROM `inspection` WHERE `status` = 'FOR VP') AS `inspection`,
											(SELECT COUNT(id) FROM `transfer_property` WHERE `status` = 'FOR VP') AS `transfer`,
											(SELECT COUNT(id) FROM `property_disposal` WHERE `status` = 'FOR VP') AS `disposal`;")
				...
				<div class="card-body">
					...
					<div class="d-flex align-items-center">
						...
						<div class="ps-3">
							...
							<a href="property_disposal_approval.php" class="badge bg-secondary text-white text-decoration-none small pt-1 fw-bold">Proceed to Disposal</a>
						</div>
					</div>
				</div>
			</div>
